...
## 2024-10-24 - Accessible Modal Triggers with Anchor Tags
**Learning:** Using `<a>` tags with `href="#"` as buttons to trigger modals breaks accessibility for screen readers and keyboard users. Screen readers will announce it as a link instead of a button, and native anchors only trigger on the `Enter` key, whereas standard buttons respond to both `Enter` and `Spacebar`.
**Action:** When using `<a>` tags for interactive actions like opening a modal, always enforce accessibility by adding `role="button"` and `aria-haspopup="dialog"`. Additionally, bind a `keydown` event listener for the `Spacebar` key that calls `e.preventDefault()` to stop page scrolling and opens the modal.
