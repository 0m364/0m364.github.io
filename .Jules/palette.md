...
## 2026-06-13 - Anchor Tag Button Accessibility
**Learning:** When using native `<a>` tags with `href="#"` as buttons to trigger actions like opening a modal, they naturally only trigger on the `Enter` key. Screen reader and keyboard users expect interactive elements acting as buttons to also respond to the `Spacebar` key.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` (if applicable) to anchor tags functioning as buttons. Additionally, bind a `keydown` event listener for the `Spacebar` key to trigger the action, ensuring to use `e.preventDefault()` to stop the default page scrolling behavior associated with `Spacebar`.
