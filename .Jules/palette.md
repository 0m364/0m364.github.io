...
## 2024-06-20 - Anchor Tags as Buttons
**Learning:** When using anchor tags (`<a href="#">`) as buttons to trigger actions like opening a modal, native browser behavior only triggers the click event on `Enter`. Screen readers and keyboard users expect button-like elements to also respond to the `Spacebar` key. Without explicitly handling the `Spacebar` keydown event, keyboard navigation is degraded and the user experiences unexpected page scrolling.
**Action:** When an anchor tag is visually and functionally behaving like a button, always add `role="button"` and `aria-haspopup="dialog"` (if opening a modal). Additionally, bind a `keydown` event listener for the `Spacebar` key (using `e.preventDefault()` to stop default page scrolling) and explicitly call the same handler function as the click event.
