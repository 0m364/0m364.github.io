...
## 2026-05-12 - Modal Trigger Accessibility with Anchors
**Learning:** When using `<a>` tags with `href="#"` to open modals, native behavior only responds to the `Enter` key. Screen readers and keyboard users expect interactive components like buttons to also trigger via the `Spacebar` key.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` to the anchor tag. Furthermore, explicitly bind a `keydown` event listener for the Spacebar (`e.key === ' ' || e.key === 'Spacebar'`), invoking `e.preventDefault()` to stop page scrolling, and then triggering the modal action.
