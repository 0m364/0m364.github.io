...
## 2026-06-13 - Anchor tags as Modal Triggers
**Learning:** Using `<a>` tags with `href="#"` as buttons to trigger modals is a common pattern, but it breaks accessibility expectations. Screen readers announce them as links, confusing users who expect a navigation action, and native links only respond to the `Enter` key, whereas standard `<button>`s also respond to the `Space` key.
**Action:** Whenever using `<a>` tags as action triggers, enforce accessibility by adding `role="button"` and `aria-haspopup="dialog"` to the HTML. Furthermore, bind a `keydown` event listener in JavaScript for the `Space` key (or `Spacebar`), calling `e.preventDefault()` to stop page scrolling before executing the modal open function.
