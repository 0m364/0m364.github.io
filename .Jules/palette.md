...
## 2026-06-13 - Anchor Tags as Buttons Accessibility
**Learning:** When using `<a>` tags with `href="#"` as buttons (like to open a modal), they lack the semantic meaning of a button for screen readers. Furthermore, native buttons can be triggered by both Enter and Spacebar keys, whereas anchor tags only respond to Enter. This discrepancy can confuse keyboard users.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` (if opening a modal) to anchor tags functioning as buttons. Additionally, bind a `keydown` event listener to handle the Spacebar key (using `e.preventDefault()` to stop page scrolling) to trigger the action, ensuring full keyboard parity with native buttons.
