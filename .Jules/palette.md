...
## 2026-04-28 - Skip to Main Content Link Target Requirements
**Learning:** When implementing a 'Skip to main content' link in a vanilla HTML/CSS project, merely adding an `id` to the target `<main>` element isn't always enough to ensure robust keyboard navigation across all browsers. If the `<main>` element isn't natively focusable, clicking the link might scroll the page visually, but the next `Tab` press might reset focus to the top of the document.
**Action:** Always add `tabindex="-1"` to the target container (e.g., `<main id="main-content" tabindex="-1">`). This allows the element to receive programmatic focus when the skip link is activated, ensuring subsequent `Tab` navigation continues logically from the main content area.
