...
## 2026-06-15 - Anchor Tags as Buttons
**Learning:** Using `<a>` tags with `href="#"` as triggers for actions (like opening a modal) is a common pattern, but it breaks accessibility for keyboard users and screen readers. Screen readers announce them as links rather than buttons, and native anchor tags only respond to the `Enter` key, whereas standard buttons respond to both `Enter` and `Spacebar`.
**Action:** When an anchor tag is used to trigger a JavaScript action instead of navigating, always add `role="button"` and `aria-haspopup="dialog"` (if opening a modal) to correctly inform assistive technologies. Additionally, bind a `keydown` event listener to handle the `Spacebar` key to match expected button interaction behavior.
