...
## 2026-05-24 - Skip to Main Content Link\n**Learning:** Keyboard-only users and screen reader users must manually tab through all navigation links before reaching the core content. Without a 'skip to main content' link, this becomes a repetitive and frustrating barrier on every page load.\n**Action:** Always include a visually hidden 'Skip to main content' link immediately after the opening `<body>` tag that becomes visible on `:focus`. Ensure the main content container has a matching `id` attribute (e.g., `<main id="main-content">`). This is a standard accessibility requirement for the project.
