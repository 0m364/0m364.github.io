...
## 2024-06-07 - Emulating Native Button Accessibility for Anchor Tags
**Learning:** When using `<a>` tags (with `href="#"`) to trigger interactive elements like modals (e.g., `OSINT` buttons), simply styling them to look like buttons isn't enough. Screen readers still announce them as links, and they do not respond to the `Spacebar` key, which keyboard users expect for buttons.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` to anchor tags used to trigger modals. Furthermore, explicitly bind a `keydown` event listener for the `Spacebar` key (and `e.preventDefault()` to stop the page from scrolling) to fully emulate native `<button>` behavior.
