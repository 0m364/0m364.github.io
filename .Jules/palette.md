...
## 2024-05-27 - Semantic Trigger Links
**Learning:** Anchor tags (`<a>` with `href="#"`) used as buttons to trigger modals do not naturally trigger on Spacebar, nor do they announce themselves as buttons to screen readers.
**Action:** When using anchor tags to open modals or trigger actions, always add `role="button"` and `aria-haspopup="dialog"`. Bind a JavaScript `keydown` listener to handle `Spacebar` (preventing default scrolling) to simulate native `<button>` functionality.
