...
## 2024-05-24 - Spacebar Accessibility for Link Buttons
**Learning:** When using anchor tags (`<a>`) as buttons to trigger actions like opening a modal, they only respond to the `Enter` key by default. Screen reader and keyboard users expect elements behaving like buttons to also trigger on the `Spacebar` key.
**Action:** Add `role="button"` and `aria-haspopup="dialog"` to the anchor tags. Implement a JavaScript `keydown` listener specifically for the `Spacebar` key (`e.key === ' '`) and call `e.preventDefault()` to prevent default page scrolling before invoking the action handler.
