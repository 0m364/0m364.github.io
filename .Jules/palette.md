...
## 2026-06-13 - Anchor-Button Triggers and Keyboard Support
**Learning:** When using `<a>` tags with `href="#"` as buttons to trigger actions like opening a modal, they lack inherent button semantics and keyboard behaviors. While native buttons respond to both the Enter and Spacebar keys, anchor tags only trigger on Enter. This creates an inconsistent and frustrating experience for keyboard and screen reader users who expect interactive elements that act like buttons to respond to Spacebar.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` to anchor tags used as modal triggers. Additionally, bind a specific JavaScript `keydown` event listener to capture the `Spacebar` key, using `e.preventDefault()` to stop the default page scroll, and manually invoke the trigger action.
