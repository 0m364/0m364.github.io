...
## 2026-06-22 - Anchor Tags as Accessible Buttons
**Learning:** When using `<a>` tags with `href="#"` as buttons (e.g., to open a modal), native anchors only respond to the 'Enter' key, but keyboard and screen reader users expect elements acting as buttons to also respond to the 'Space' key. Additionally, screen readers announce these elements as generic links unless explicitly defined.
**Action:** Always add `role="button"` to anchor tags functioning as buttons. If they open a modal, also include `aria-haspopup="dialog"`. Crucially, bind a JavaScript `keydown` event listener for the `Space` key that calls `e.preventDefault()` (to stop the page from scrolling down) and then executes the click action.
