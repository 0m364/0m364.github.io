...
## 2024-05-24 - Trigger Anchor Tags for Modals
**Learning:** Using `<a>` tags with `href="#"` to open modals fails to correctly communicate the element's purpose to screen readers and is fundamentally inaccessible for keyboard users who rely on the spacebar to activate buttons.
**Action:** When using `<a>` tags to trigger modals, enforce accessibility by adding `role="button"` and `aria-haspopup="dialog"`. Additionally, bind a `keydown` event listener for the spacebar (`e.key === ' ' || e.key === 'Spacebar'`), explicitly calling `e.preventDefault()` to stop page scrolling before executing the modal open logic.
