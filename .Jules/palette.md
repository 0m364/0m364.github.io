...
## 2026-06-25 - Anchor Tags as Buttons
**Learning:** When using anchor tags (`<a>`) as buttons to open modals or trigger JavaScript actions, they lack native keyboard support for the `Spacebar` key, unlike true `<button>` elements. Screen readers also may not announce them correctly without `role="button"` and `aria-haspopup="dialog"`.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` to anchor tags used as modal triggers. Additionally, bind a `keydown` event listener for the `Spacebar` key (`e.key === ' ' || e.code === 'Space'`), calling `e.preventDefault()` to stop page scrolling and invoking the action handler.
