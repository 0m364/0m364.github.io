...
## 2026-05-18 - Skip to Main Content Link
**Learning:** For keyboard-only users, traversing the main navigation repeatedly on every page becomes tedious. A globally available "Skip to main content" link at the very beginning of the document structure significantly improves keyboard navigation accessibility by allowing users to bypass repetitive header elements.
**Action:** Always include a visually hidden "Skip to main content" link immediately after the opening `<body>` tag that becomes visible on `:focus`. Ensure the main content container has a matching `id` attribute (e.g., `<main id="main-content">`). This should be a foundational accessibility requirement for all pages.
