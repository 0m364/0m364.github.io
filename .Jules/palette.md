...
**Learning:** When using `<a>` tags with `href="#"` as interactive elements to trigger actions (like opening a modal), screen readers treat them as links rather than buttons. Additionally, native anchors only trigger on `Enter`, whereas screen readers and keyboard users expect buttons to trigger on both `Enter` and `Spacebar`.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` (if applicable) to anchor tags acting as buttons. Bind a JavaScript `keydown` event listener for the `Spacebar` key (using `e.preventDefault()` to stop default page scrolling) to ensure full keyboard functionality.
