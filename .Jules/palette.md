...
## 2024-04-20 - Skip to Main Content Links
**Learning:** Users relying on keyboard navigation (like Tab) have to navigate through the entire header/navigation menu on every page load to reach the main content. This is tedious and a poor experience.
**Action:** Always include a visually hidden "Skip to main content" link immediately after the opening `<body>` tag that becomes visible on `:focus`. Ensure the main content container has a matching `id` attribute (e.g. `<main id="main-content">`) so the anchor link can properly shift focus.
