...
## 2026-06-12 - Anchors as Buttons Accessibility
**Learning:** When using anchor tags (`<a href="#">`) styled as buttons to trigger actions like opening a modal, they inherently lack the correct semantic role for screen readers, and they do not respond to the `Space` key like native buttons do.
**Action:** Always add `role="button"` to anchors used for actions, and explicitly define `aria-haspopup` if they open a modal or menu. Additionally, bind a `keydown` listener for the `Space` key (calling `e.preventDefault()` to avoid scrolling) to match the keyboard interaction expected from a native `<button>`.
