...
## 2024-05-29 - Links as Buttons Accessibility
**Learning:** When using `<a>` tags with `href="#"` to act as buttons (e.g., opening a modal), they only respond to the `Enter` key natively. Screen readers and keyboard users expect button elements to respond to both `Enter` and `Spacebar`.
**Action:** Always add `role="button"` and `aria-haspopup="dialog"` (when applicable) to anchor elements acting as buttons. Additionally, bind a `keydown` event listener for the `Spacebar` key and explicitly call `e.preventDefault()` to stop the default behavior of scrolling the page before triggering the action.
