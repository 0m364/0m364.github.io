...
## 2026-06-23 - Anchor Tags as Buttons Accessibility
**Learning:** Using `<a>` tags with `href="#"` as buttons to trigger actions like opening a modal is a common pattern but presents accessibility gaps. Screen readers announce them as links rather than buttons, and native anchor tags do not trigger via the 'Spacebar' key like native buttons do.
**Action:** When using anchor tags as buttons, enforce accessibility by adding `role="button"` and `aria-haspopup="dialog"`. Additionally, bind a JavaScript `keydown` event listener for the `Spacebar` key (using `e.preventDefault()` to stop default page scrolling), as native anchors only trigger on `Enter`, whereas screen readers and keyboard users expect buttons to respond to both.
