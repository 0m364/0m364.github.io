...
## 2026-06-13 - Anchor Tag as Button Accessibility
**Learning:** Using `<a>` tags with `href="#"` as buttons to trigger modals breaks accessibility because native anchors only trigger on the `Enter` key, whereas screen readers and keyboard users expect buttons to respond to both `Enter` and `Spacebar`. Furthermore, without `role="button"` and `aria-haspopup="dialog"`, screen readers do not announce the element's purpose correctly.
**Action:** When using `<a>` tags to trigger modals, always add `role="button"` and `aria-haspopup="dialog"`. Additionally, bind a JavaScript `keydown` event listener for the `Spacebar` key (using `e.preventDefault()` to stop default page scrolling) to emulate native button behavior.
